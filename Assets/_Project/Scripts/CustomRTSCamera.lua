...
--!SerializeField
local orthographic : boolean = false
...
local CameraMoveStateEnum = {
    ...
    CenteringOnTarget = 4; -- Added state
}
...
-- Variables for SmoothCenterOnTarget
local centeringTargetPosition : Vector3 = Vector3.zero
local centeringTargetYaw : number = 0
local centeringTargetPitch : number = 0
local centeringTargetZoom : number = 0
local centeringStartTime : number = 0
local centeringDuration : number = 1.0
...
client.Reset:Connect(function(evt)
    ...
    -- Reset centering target variables for standard reset
    centeringTargetPosition = client.localPlayer.character.gameObject.transform.position
    centeringTargetYaw = yaw -- Use default yaw for standard reset
    centeringTargetPitch = pitch -- Use default pitch for standard reset
    centeringTargetZoom = defaultZoom
    resetLerpDuration = 1.2 -- Use default duration for standard reset
...
Input.PinchOrDragBegan:Connect(function(evt)
    ...
    cameraMoveState = CameraMoveStateEnum.ManualControl -- Interrupt centering
...
Input.PinchOrDragEnded:Connect(function(evt)
    ...
    -- Only switch to None if we were manually controlling
...
function PostZoomMoveTowardsScreenPoint(screenPosition)
    if orthographic == true then 
        -- orthographic zoom just modifies the 'size' of the frustum, no need to
        -- modify the position
        return
    end
    local ray = camera:ScreenPointToRay(screenPosition)
...
function CenterOn(newTarget, newZoom)
    -- This function provides an immediate jump, kept for potential other uses
    cameraMoveState = CameraMoveStateEnum.None -- Ensure immediate jump isn't overridden
    ResetInertia()
    zoom = newZoom or zoom
    target = newTarget
    zoom = Mathf.Clamp(zoom, zoomMin, zoomMax)
    -- Optionally reset rotation here if needed for an instant center
    -- rotation.x = 0
    -- rotation.y = 0
    UpdatePosition() -- Apply immediately
end

function self:SmoothCenterOnTarget(targetTransform : Transform, newZoom : number, duration : number)
    if not self:IsActive() then return end

    ResetInertia()
    cameraMoveState = CameraMoveStateEnum.CenteringOnTarget

    centeringTargetPosition = targetTransform.position
    -- Calculate yaw to look towards the target's position from behind it, based on its forward vector
    local targetForward = targetTransform.forward
    targetForward.y = 0 -- Project onto the horizontal plane
    local desiredCamDir = -targetForward.normalized -- Camera should be behind the target
    centeringTargetYaw = Quaternion.LookRotation(desiredCamDir, Vector3.up).eulerAngles.y - yaw -- Adjust relative to default yaw

    centeringTargetPitch = 0 -- Reset pitch to default look slightly down angle (relative to default pitch)
    centeringTargetZoom = Mathf.Clamp(newZoom or zoom, zoomMin, zoomMax)
    centeringDuration = duration or 1.0
...
function self:Update()
    ...
    if cameraMoveState == CameraMoveStateEnum.None or cameraMoveState == CameraMoveStateEnum.PlayerOffScreenFollow then
        if keepPlayerInView and cameraMoveState ~= CameraMoveStateEnum.PlayerOffScreenFollow then -- Avoid conflict if already panning
            PanIfPlayerOutOfView();
        end
    elseif cameraMoveState == CameraMoveStateEnum.Resetting then
        local lerp = Mathf.Clamp01((Time.time - resetTime) / resetLerpDuration)

        -- Use centeringTarget variables which are set appropriately in client.Reset:Connect
        target = Vector3.Lerp(target, centeringTargetPosition, lerp)
        rotation.x = Mathf.LerpAngle(rotation.x, centeringTargetPitch, lerp)
        rotation.y = Mathf.LerpAngle(rotation.y, centeringTargetYaw, lerp)
        zoom = Mathf.Lerp(zoom, centeringTargetZoom, lerp)

        if (lerp >= 1) then
            cameraMoveState = CameraMoveStateEnum.None
        end
    elseif cameraMoveState == CameraMoveStateEnum.CenteringOnTarget then
        local lerp = Mathf.Clamp01((Time.time - centeringStartTime) / centeringDuration)

        target = Vector3.Lerp(target, centeringTargetPosition, lerp)
        rotation.x = Mathf.LerpAngle(rotation.x, centeringTargetPitch, lerp) -- Lerp towards default pitch
        rotation.y = Mathf.LerpAngle(rotation.y, centeringTargetYaw, lerp)   -- Lerp towards calculated yaw
        zoom = Mathf.Lerp(zoom, centeringTargetZoom, lerp)
    ...
    -- Apply Inertia only if not actively controlled or transitioning
    if cameraMoveState == CameraMoveStateEnum.None or cameraMoveState == CameraMoveStateEnum.PlayerOffScreenFollow then
        UpdateInertia()
    end

    -- Apply centering on character movement if enabled and not in a manual/transition state
    if centerOnCharacterWhenMovingSpeed > 0 and IsCharacterMoving() and
       (cameraMoveState == CameraMoveStateEnum.None or cameraMoveState == CameraMoveStateEnum.PlayerOffScreenFollow) then
